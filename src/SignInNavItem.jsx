      ...
      fetch('/signin', {
        ...
        if (response.ok) {
          ...
          response.json().then(error => {
            this.props.showError(`App loging failed: ${error}`);
          });
        }
      })
      .catch(err => {
        this.props.showError(`Error posting loging to app: ${err}`);
      });
    }, error => {
    ...
    return (
      <NavItem onClick={this.showModal}>Sign in
        <Modal keyboard show={this.state.showing} onHide={this.hideModal} bsSize="sm">
          ...
          <Modal.Footer>
            <Button bsStyle="link" onClick={this.hideModal}>Cansel</Button>
          </Modal.Footer>
        </Modal>
      </NavItem>
    );
